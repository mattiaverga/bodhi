...
def _expected_skopeo_args(config, source, destination, dtag, extra_args=[]):
    ...
    return (
        ...
        + extra_args
        + [
            "docker://{}/{}@sha256:{}".format(
                config["container.source_registry"], source_repository, source_digest
            ),
...
def _expected_skopeo_call(config, source, destination, dtag, extra_args=[]):
    return mock.call(
        _expected_skopeo_args(config, source, destination, dtag, extra_args=extra_args),
        shell=False,
        stderr=-1,
        stdout=-1,
        cwd=None,
...
def _make_task(transactional_session_maker, extra_push_args=None):
    ...
    with mock.patch("bodhi.server.push.compose_task") as compose, mock.patch(
...
class TestComposer(base.BasePyTestCase):
    ...
    def setup_method(self, method):
        ...
        # We don't really need this to be cleaned up since we don't want any tests launching
        # threads anyway, so we don't bother with mock.
        ...
        config.update(
            {
                ...
                "pungi.basepath": os.path.join(base.PROJECT_PATH, "tests/consumers/pungi.basepath"),
...
<revision>1508375628</revision></repomd>"""
            ...
            # We need to fake Pungi having run or _wait_for_pungi() will fail to find the
            # output dir
            ...
            with open(
                os.path.join(compose_dir, "compose", "metadata", "composeinfo.json"), "w"
            ) as f:
                f.write("{}")
...
That was the actual one"""
    ...
    def test___init___missing_paths(self, mock_os_path_exists):
        ...
        for s in ("pungi.cmd", "compose_dir", "compose_stage_dir"):
            ...
            assert f"'/does/really/not/exist' does not exist. Check the {s} setting." in str(
                exc.value
            )
    ...
    def test__get_composes_api_2(self):
        ...
        with self.db_factory() as db:
            ...
            assert composes == [
                {
                    "content_type": compose.content_type.value,
                    "release_id": compose.release.id,
                    "request": compose.request.value,
    ...
    def test__get_composes_not_found(self, info):
        ...
        that will lead to Nack loops. If we receive a message like this, _get_composes() should
        just return the empty list. See https://github.com/fedora-infra/bodhi/issues/3318
        """
        task = self._make_task()
        api_version = task.pop("api_version")
    ...
    def test_update_locking(self, *args):
        ...
        expected_messages = (
            ...
            compose_schemas.ComposeCompleteV1.from_dict(
                dict(success=False, repo="f17-updates-testing", ctype="rpm", agent="bowlofeggs")
            ),
        )
    ...
    def test_tags(self, *args):
        ...
        expected_messages = (
            ...
            compose_schemas.ComposeCompleteV1.from_dict(
                dict(success=True, repo="f17-updates-testing", ctype="rpm", agent="bowlofeggs")
            ),
        )
        ...
        with self.db_factory() as session:
            ...
            self.koji.__tagged__[session.query(Update).first().title] = [
        ...
        expected_messages = (
            ...
            compose_schemas.ComposeCompleteV1.from_dict(
                dict(success=True, repo="f17-updates", ctype="rpm", agent="bowlofeggs")
            ),
        )
    ...
    def test_tag_ordering(self, *args):
        ...
        with self.db_factory() as session, mock.patch("bodhi.server.models.notifications"):
            ...
            build = RpmBuild(
                nvr=otherbuild,
                package=firstupdate.builds[0].package,
    ...
    def test_testing_digest(self, mail, *args):
        ...
        task = self._make_task()
        t = RPMComposerThread(
            self.semmock, task["composes"][0], "ralph", self.db_factory, self.tempdir
        )
        ...
        assert (
            t.testing_digest["Fedora 17"]["bodhi-2.0-1.fc17"]
            == f"""\
================================================================================
...
"""
        ...
        mail.assert_called_with(
            config.get("bodhi_email"), config.get("fedora_test_announce_list"), mock.ANY
        )
        ...
        assert body.startswith(
            (
                ...
                "pushed to Fedora 17 updates-testing\n\n    bodhi-2.0-1.fc17\n\nDetails about "
                "builds:\n\n\n==================================================================="
                "=============\n libseccomp-2.1.0-1.fc20 (FEDORA-%s-a3bbe1a8f2)\n Enhanced "
                "seccomp library\n---------------------------------------------------------------"
                "-----------------\nUpdate Information:\n\nUseful details!\n---------------------"
                "-----------------------------------------------------------\nChangeLog:\n\n* Sat "
                "Aug  3 2013 Fedora Release Engineering <rel-eng@lists.fedoraproject.org> - "
                "2.1.0-1\n- Rebuilt for https://fedoraproject.org/wiki/Fedora_20_Mass_Rebuild\n"
                "* Tue Jun 11 2013 Paul Moore <pmoore@redhat.com> - 2.1.0-0\n- New upstream "
                "version\n- Added support for the ARM architecture\n- Added the scmp_sys_resolver "
                "tool\n* Mon Jan 28 2013 Paul Moore <pmoore@redhat.com> - 2.0.0-0\n- New upstream "
                "version\n* Tue Nov 13 2012 Paul Moore <pmoore@redhat.com> - 1.0.1-0\n- New "
                "upstream version with several important fixes\n* Tue Jul 31 2012 Paul Moore "
                "<pmoore@redhat.com> - 1.0.0-0\n- New upstream version\n- Remove verbose build "
                "patch as it is no longer needed\n- Enable _smp_mflags during build stage\n* Thu "
                "Jul 19 2012 Fedora Release Engineering <rel-eng@lists.fedoraproject.org> - "
                "0.1.0-2\n- Rebuilt for https://fedoraproject.org/wiki/Fedora_18_Mass_Rebuild\n* "
                "Tue Jul 10 2012 Paul Moore <pmoore@redhat.com> - 0.1.0-1\n- Limit package to "
                "x86/x86_64 platforms (RHBZ #837888)\n* Tue Jun 12 2012 Paul Moore "
                "<pmoore@redhat.com> - 0.1.0-0\n- Initial version\n------------------------------"
                "--------------------------------------------------\nReferences:\n\n  [ 1 ] Bug "
                "#12345 - None\n        https://bugzilla.redhat.com/show_bug.cgi?id=12345\n------"
                "--------------------------------------------------------------------------\n\n"
            )
...
That was the actual one"""
            ...
            expected_error = "Directory at /tmp/nonsensical_directory does not look like a compose"
    ...
    def test_sanity_check_empty_dir(self, save_state):
        ...
        with self.db_factory() as session:
            ...
            t._startyear = datetime.now(timezone.utc).year
            t._wait_for_pungi(
                self._generate_fake_pungi(
                    t, "testing_tag", t.compose.release, empty=True, noarches=True
                )()
            )
            t.db = None
    ...
    def test_sanity_check_no_arches(self, save_state):
        ...
        with self.db_factory() as session:
            ...
            t._startyear = datetime.now(timezone.utc).year
            t._wait_for_pungi(
                self._generate_fake_pungi(t, "testing_tag", t.compose.release, noarches=True)()
            )
            t.db = None
    ...
    def test_sanity_check_valid(self, save_state):
        ...
        base.mkmetadatadir(
            os.path.join(t.path, "compose", "Everything", "source", "tree"), source=True
        )
        os.makedirs(
            os.path.join(t.path, "compose", "Everything", "source", "tree", "Packages", "a")
        )
        with open(
            os.path.join(
                t.path, "compose", "Everything", "source", "tree", "Packages", "a", "test.src.rpm"
            ),
            "w",
        ) as tf:
            tf.write("bar")
    ...
    def test_sanity_check_broken_repodata(self, save_state):
        ...
        for arch in ("i386", "x86_64", "armhfp"):
            ...
            xml = os.path.join(
                t.path, "compose", "Everything", arch, "os", "repodata", "repomd.xml"
            )
            with open(xml) as f:
                repomd = f.read()
    ...
    def test_security_update_priority(self, *args):
        ...
        expected_messages = (
            ...
            errata_schemas.ErrataPublishV1,
            compose_schemas.ComposeCompleteV1.from_dict(
                dict(success=True, repo="f18-updates", ctype="rpm", agent="bowlofeggs")
            ),
            compose_schemas.ComposeComposingV1.from_dict(
                {
                    "repo": "f17-updates-testing",
        ...
        with self.db_factory() as db:
            ...
            db.add(release)
            build = RpmBuild(
                nvr="bodhi-2.0-1.fc18", release=release, package=up.builds[0].package, signed=True
            )
            db.add(build)
            update = Update(
                builds=[build],
    ...
    def test_compose_invalid_ctype(self, mocked_log, *args):
        ...
        with mock.patch.object(self.handler, "_get_composes", return_value=task["composes"]):
    ...
    def test_clean_old_composes_param(self, sleep, wfs, wfrs, stage, scr, clean_old):
        ...
        t = RPMComposerThread(
            self.semmock, task["composes"][0], "ralph", self.db_factory, compose_dir
        )
        ...
        expected_messages = (
            ...
            errata_schemas.ErrataPublishV1,
            compose_schemas.ComposeCompleteV1.from_dict(
                dict(success=True, repo="f17-updates", ctype="rpm", agent="ralph")
            ),
        )
        ...
        actual_files = [
            f for f in os.listdir(compose_dir) if os.path.isfile(os.path.join(compose_dir, f))
        ]
        ...
        assert Popen.mock_calls == [
            mock.call(
                [
                    config["pungi.cmd"],
                    "--config",
                    f"{t._pungi_conf_dir}/pungi.conf",
                    "--quiet",
                    "--print-output-dir",
                    "--target-dir",
                    t.compose_dir,
    ...
    def test_compose_module(self, *args):
        ...
        with self.db_factory() as db:
            ...
            package = Package(name="testmodule", type=ContentType.module)
            db.add(package)
            build1 = ModuleBuild(
                nvr="testmodule-master-20171.1", release=release, signed=True, package=package
            )
            db.add(build1)
            build2 = ModuleBuild(
                nvr="testmodule-master-20172.2", release=release, signed=True, package=package
            )
            db.add(build2)
            update = Update(
                builds=[build2],
        ...
        t = ModuleComposerThread(
            self.semmock, task["composes"][0], "puiterwijk", self.db_factory, self.tempdir
        )
        expected_messages = (
            compose_schemas.ComposeComposingV1,
            update_schemas.UpdateCompleteStableV1,
            errata_schemas.ErrataPublishV1,
            compose_schemas.ComposeCompleteV1.from_dict(
                dict(success=True, repo="f27M-updates", ctype="module", agent="puiterwijk")
            ),
        )
        ...
        assert list(t._module_defs) == [
            {"context": "2", "version": "20172", "name": "testmodule", "stream": "master"}
        ]
...
"""
        ...
        assert t._checkpoints == {
            "completed_repo": os.path.join(
                self.tempdir, f"Fedora-27-updates-{d.year}{d.month:02}{d.day:02}.0"
            ),
            "compose_done": True,
            "determine_and_perform_tag_actions": True,
            "modify_bugs": True,
    ...
    def test_compose_module_koji_multicall_result_empty_list(self):
        ...
        package = Package(name="testmodule", type=ContentType.module)
        build = ModuleBuild(
            nvr="testmodule-master-20171", release=release, signed=True, package=package
        )
        t = ModuleComposerThread(self.semmock, {}, "puiterwijk", log, self.db_factory, self.tempdir)
        with pytest.raises(Exception) as exc:
            t._raise_on_get_build_multicall_error([], build)

        assert f'Empty list returned for getBuild("{build.nvr}").' in str(exc.value)
    ...
    def test_compose_module_koji_multicall_result_dict(self):
        ...
        assert f'Unexpected data returned for getBuild("{build.nvr}"): {{}}.' in str(exc.value)
    ...
    def test_test_gating_status_failed(self, *args):
        ...
        assert u.comments[-1].text == (
            f"{u.alias} ejected from the push because 'Required tests did not pass on this update.'"
        )
    ...
    def test_test_gating_status_passed(self, *args):
        ...
        expected_messages = (
            ...
            compose_schemas.ComposeComposingV1.from_dict(
                {
                    "repo": "f17-updates",
                    "updates": [u.builds[0].nvr],
                    "agent": "ralph",
                    "ctype": "rpm",
                }
            ),
            override_schemas.BuildrootOverrideUntagV1.from_dict(
                dict(override=u.builds[0].override.__json__())
            ),
            update_schemas.UpdateCompleteStableV1,
            errata_schemas.ErrataPublishV1,
            compose_schemas.ComposeCompleteV1.from_dict(
                dict(success=True, ctype="rpm", repo="f17-updates", agent="ralph")
            ),
        )
    ...
    def test_modify_testing_bugs(self, on_qa, modified, *args):
        ...
        expected_message = expected_message.format(
            time.strftime("%Y"),
            urlparse.urljoin(
                config["base_address"], f"/updates/FEDORA-{time.strftime('%Y')}-a3bbe1a8f2"
            ),
        )
    ...
    def test_modify_stable_bugs(self, close, comment, *args):
        ...
        close.assert_called_with(
            ...
            comment=(
                f"FEDORA-{time.strftime('%Y')}-a3bbe1a8f2 (bodhi-2.0-1.fc17) has been "
    ...
    @mock.patch(
        "bodhi.server.tasks.composer.PungiComposerThread._sanity_check_repo",
    ...
    def test_retry_done_compose(
        self,
        mock_cmd,
        sleep,
        mock_wait_for_sync,
        mock_generate_updateinfo,
        mock_wait_for_repo_signature,
        mock_stage_repo,
        mock_sanity_check_repo,
    ):
        self.expected_sems = 2
    ...
    def test_obsolete_older_updates(self, *args):
        ...
        with self.db_factory() as session:
            ...
            build = RpmBuild(
                nvr=otherbuild,
                package=oldupdate.builds[0].package,
    ...
    def test_expire_buildroot_overrides_exception(self, expire, exception_log, *args):
        ...
        with self.db_factory() as db:
            ...
            pending_testing_tag = release.pending_testing_tag
            self.koji.__tagged__[db.query(Update).first().title] = [
                release.override_tag,
                pending_testing_tag,
            ]
            up = db.query(Update).one()
...
class TestContainerComposerThread__compose_updates(ComposerThreadBaseTestCase):
    ...
    def setup_method(self, method):
        ...
        release.branch = "f28"
        package1 = Package(name="testcontainer1", type=ContentType.container)
        self.db.add(package1)
        package2 = Package(name="testcontainer2", type=ContentType.container)
        self.db.add(package2)
        build1 = ContainerBuild(
            nvr="testcontainer1-2.0.1-71.fc28container",
            release=release,
            signed=True,
            package=package1,
        )
        self.db.add(build1)
        build2 = ContainerBuild(
            nvr="testcontainer2-1.0.1-1.fc28container",
            release=release,
            signed=True,
            package=package2,
        )
    ...
    def test_request_not_stable(self, Popen):
        ...
        t = ContainerComposerThread(
            self.semmock, task["composes"][0], "bowlofeggs", self.Session, self.tempdir
        )
    ...
    def test_skopeo_error_code(self, Popen):
        ...
        skopeo_cmd = _expected_skopeo_args(
            config,
            "f28/testcontainer1:2.0.1-71.fc28container",
    ...
    def test_skopeo_extra_copy_flags(self, Popen):
        ...
        for source in (
            ...
            for dtag in [source.split(":")[1], source.split(":")[1].split("-")[0], "testing"]:
                for destination in config["container.destination_registry"]:
                    mock_call = _expected_skopeo_call(
                        config, source, destination, dtag, extra_args=["--dest-tls-verify=false"]
                    )
                    expected_mock_calls.append(mock_call)
                    expected_mock_calls.append(mock.call().communicate())
        assert Popen.mock_calls == expected_mock_calls
...
class TestPungiComposerThread__compose_updates(ComposerThreadBaseTestCase):
    ...
    def test_compose_dir_dne(self):
        ...
        t = PungiComposerThread(
            self.semmock, task["composes"][0], "bowlofeggs", self.Session, compose_dir
        )
    ...
    def test_pungi_cleaned_up_on_exception(self):
        ...
        with (
            mock.patch.object(t, "_punge", return_value=pungi_process),
            mock.patch.object(t, "generate_testing_digest", side_effect=Exception("koji is sad")),
...
class TestFlatpakComposerThread__compose_updates(ComposerThreadBaseTestCase):
    ...
    def setup_method(self, method):
        ...
        release.branch = "f28"
        package1 = Package(name="testflatpak1", type=ContentType.flatpak)
        self.db.add(package1)
        package2 = Package(name="testflatpak2", type=ContentType.flatpak)
        self.db.add(package2)
        build1 = FlatpakBuild(
            nvr="testflatpak1-2.0.1-71.fc28flatpak", release=release, signed=True, package=package1
        )
        self.db.add(build1)
        build2 = FlatpakBuild(
            nvr="testflatpak2-1.0.1-1.fc28flatpak", release=release, signed=True, package=package2
        )
        self.db.add(build2)
        update = Update(
            builds=[build1, build2],
    ...
    def test_flatpak_compose(self, Popen):
        ...
        t = FlatpakComposerThread(
            self.semmock, task["composes"][0], "otaylor", self.Session, self.tempdir
        )
        t.compose = Compose.from_dict(self.db, task["composes"][0])
...
class TestPungiComposerThread__get_master_repomd_url(ComposerThreadBaseTestCase):
        ...
        config.update(
            {
                ...
                "fedora_testing_alt_master_repomd": (
                    "http://example.com/pub/fedora-secondary/updates/testing/%s/%s/"
                    "repodata.repomd.xml"
                ),
            }
        )
    ...
    def test_master_repomd_undefined(self):
        ...
        assert (
            ...
            "fedora_testing_alt_master_repomd in the config file" in str(exc.value)
        )
    ...
    def test_primary_arch_version_override(self):
        ...
        assert (
            url == "http://example.com/pub/fedora/linux/updates/testing/17/Everything/"
...
class TestComposerThread_check_all_karma_thresholds(ComposerThreadBaseTestCase):
    ...
    @mock.patch(
        "bodhi.server.models.Update.check_karma_thresholds",
    ...
    def test_from_tag_not_found(self, get_session):
        ...
        expected_messages = (
            update_schemas.UpdateEjectV1.from_dict(
                {
                    "repo": "f17-updates",
                    "update": self.db.query(Update).one().__json__(),
                    "reason": (
                        f"Cannot find relevant tag for bodhi-2.0-1.fc17.  None of {tags} are "
                        f"in {Release.get_tags()[0]['candidate']}."
                    ),
                    "request": UpdateRequest.testing,
                    "release": t.compose.release,
                    "agent": "bowlofeggs",
                }
            ),
        )
...
class TestComposerThread_remove_state(ComposerThreadBaseTestCase):
    ...
    def test_remove_state(self):
        ...
        t = ComposerThread(
            self.semmock, self._make_task()["composes"][0], "bowlofeggs", self.Session, self.tempdir
        )
        t.compose = self.db.query(Compose).one()
        t.db = self.db
...
class TestPungiComposerThread__wait_for_sync(ComposerThreadBaseTestCase):
    ...
    @mock.patch(
        "bodhi.server.tasks.composer.time.sleep",
    ...
    def test_checksum_match_third_try(self, urlopen, sleep, save):
        ...
        expected_calls = [
            mock.call(
                f"http://example.com/pub/fedora/linux/updates/testing/17/{arch}/repodata.repomd.xml"
            ),
            mock.call().read(),
        ]
    ...
    def test_incompleteread(self, mocked_log, urlopen, sleep, save):
        ...
        for i in range(2):
            expected_calls.append(
                mock.call(
                    "http://example.com/pub/fedora/linux/updates/testing/17/"
    ...
    @mock.patch(
        "bodhi.server.tasks.composer.urlopen",
    ...
    def test_missing_config_key(self, save):
        ...
        with pytest.raises(ValueError) as exc, mock_sends(
            compose_schemas.ComposeSyncWaitV1.from_dict({"repo": t.id, "agent": "bowlofeggs"})
        ):
            t._wait_for_sync()
        ...
        assert (
            ...
            "fedora_testing_master_repomd in the config file" in str(exc.value)
        )
        save.assert_called_once_with(ComposeState.syncing_repo)
...
class TestPungiComposerThread__stage_repo(ComposerThreadBaseTestCase):
    ...
    def test_old_link_present(self, mocked_log):
        ...
        mocked_log.info = mock.MagicMock()
        t = PungiComposerThread(
            self.semmock, self._make_task()["composes"][0], "ralph", self.Session, self.tempdir
        )
        t.id = "f17-updates-testing"
        ...
        assert mocked_log.info.mock_calls == [mock.call(f"Creating symlink: {link} => {t.path}")]
...
class TestPungiComposerThread__wait_for_repo_signature(ComposerThreadBaseTestCase):
    ...
    def test_dont_wait_for_signatures(self, mocked_log):
        ...
        with mock_sends(
            compose_schemas.RepoDoneV1.from_dict({"repo": t.id, "path": t.path, "agent": "ralph"})
        ):
            t._wait_for_repo_signature()

        assert mocked_log.info.mock_calls == [mock.call("Not waiting for a repo signature")]
