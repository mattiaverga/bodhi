...
class TestSendMail:
    ...
    @mock.patch.dict(
        "bodhi.server.mail.config", {"exclude_mail": ["fbi@watchingy.ou", "nsa@spies.biz"]}
    )
...
class TestSendReleng:
    ...
    def test_send_releng(self, SMTP):
        ...
        smtp.sendmail.assert_called_once_with(
            ...
            .format(config.config.get("bodhi_email"), config.config.get("release_team_address"))
            .encode("utf-8"),
        )
