...
%(updatestr)s
""",
        "fields": lambda agent, x: {"submitter": agent, "updatestr": str(x)},
    },
    "unpush": {
        "body": """\
...
""",
        "fields": lambda agent, x: {"updatestr": str(x)},
    },
    "revoke": {
        "body": """\
...
%(updatestr)s
""",
        "fields": lambda agent, x: {"karma": x.karma, "updatestr": str(x)},
    },
    "unstable": {
        "body": """\
The following update has reached a karma of %(karma)d and is being
...
""",
        "fields": lambda agent, x: {"package": x.title, "submitter": agent, "updatestr": str(x)},
    },
}
...
def send_mail(
    from_addr: str, to_addr: str, subject: str, body_text: str, headers: dict | None = None
) -> None:
    """
    Send an e-mail.
...
def send_releng(subject: str, body: str) -> None:
    ...
    Args:
        ...
        body: The body of the e-mail.
    """
    send_mail(config.get("bodhi_email"), config.get("release_team_address"), subject, body)
